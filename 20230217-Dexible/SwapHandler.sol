...
abstract contract SwapHandler is AdminBase, ISwapHandler {
    ...
    function fill(
        ...
        require(
            meta.outAmount >= request.tokenOut.amount,
    ...
    function postFill(
        SwapTypes.SwapRequest memory request,
        SwapMeta memory meta,
        bool success
    ) internal {
        if (success) {
    ...
    function handleSwapFailure(
        ...
        if (meta.feeIsInput) {
            unchecked {
                ...
                console.log(
                    "Estimated gas used for failed gas payment",
                    totalGas
                );
                meta.nativeGasAmount = LibFees.computeGasCost(totalGas, false);
            }

            gasInFeeToken = dd.communityVault.convertGasToFeeToken(
                address(request.executionRequest.fee.feeToken),
                meta.nativeGasAmount
            );
            ...
            request.executionRequest.fee.feeToken.safeTransferFrom(
    ...
    function collectDXBL(
        ...
        dd.communityVault.rewardTrader(
            request.executionRequest.requester,
            address(request.executionRequest.fee.feeToken),
            value
        );
     ...
     */
    function allocateRevshareAndAffiliate(
        SwapTypes.SwapRequest memory request,
        SwapMeta memory meta
    ) internal view {
        DexibleStorage.DexibleData storage dd = DexibleStorage.load();
        ...
        uint minFee = LibFees.computeMinFeeUnits(
            address(request.executionRequest.fee.feeToken)
        );
        ...
        require(
            request.executionRequest.fee.affiliatePortion <
                meta.bpsAmount - meta.toRevshare,
            "Miscalculated affiliate portion"
        );
        meta.toProtocol =
            (meta.bpsAmount - meta.toRevshare) -
            request.executionRequest.fee.affiliatePortion;
            ...
            if (meta.outAmount < total) {
                revert(
                    string(
                        abi.encodePacked(
                            _concatUintString(
                                "Insufficient output to pay bps fees. Required: ",
                                total
                            ),
                            _concatUintString(
                                " Output amount: ",
    ...
    function payProtocolAndTrader(
        ...
        if (!meta.isSelfSwap) {
            ...
            unchecked {
                ...
                uint totalGas = (meta.startGas - gasleft());
                if (
                    address(dd.communityVault) != meta.preSwapVault &&
                    totalGas > 200_000
                ) {
                ...
                console.log(
                    "Estimated gas used for trader gas payment",
                    totalGas
                );
            ...
            meta.gasAmount = dd.communityVault.convertGasToFeeToken(
                address(request.executionRequest.fee.feeToken),
                meta.nativeGasAmount
            );
                ...
                if (meta.outToTrader <= meta.gasAmount) {
                    revert(
                        string(
                            abi.encodePacked(
                                _concatUintString(
                                    "Insufficient output to pay gas fees. Required: ",
                                    meta.gasAmount
                                ),
                                _concatUintString(
                                    " Trader output proceeds: ",
                                    meta.outToTrader
                                )
                            )
                        )
        ...
        if (meta.feeIsInput) {
            ...
            //fees
            uint totalInputSpent = request.routes[0].routeAmount.amount +
                meta.inputAmountDue;
            ...
            if (totalInputSpent > request.tokenIn.amount) {
                revert(
                    string(
                        abi.encodePacked(
                            _concatUintString(
                                "Attempt to spend more input than anticipated. Total required: ",
                                totalInputSpent
                            ),
                            _concatUintString(
                                " Max input: ",
                                request.tokenIn.amount
                            )
                        )
                    )
                );
        ...
        request.tokenOut.token.safeTransfer(
            request.executionRequest.requester,
            meta.outToTrader
        );
    ...
    function preCheck(
        ...
        require(
            fToken == address(request.tokenIn.token) ||
        ...
        meta.preDXBLBalance = dd.dxblToken.balanceOf(
            request.executionRequest.requester
        );

        //flag whether the input token is the fee token
        meta.feeIsInput =
            address(request.tokenIn.token) ==
            address(request.executionRequest.fee.feeToken);
