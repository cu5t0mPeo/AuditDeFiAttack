    ...
    ICommunityVault
{
    ...
    constructor(VaultStorage.VaultConfig memory config) {
        ...
        if (vs.adminMultiSig != address(0)) {
            require(
                msg.sender == vs.adminMultiSig,
                "Unauthorized to change configuration"
            );
        } else {
        ...
        require(
            config.baseMintThreshold > 0,
            "Must provide a base mint threshold"
        );
        require(
            config.wrappedNativeToken != address(0),
            "Invalid wrapped native token"
        );
    ...
    receive() external payable {
        require(
            msg.sender == VaultStorage.load().wrappedNativeToken,
            "Cannot receive funds except for native withdraws"
        );
    }
    ...
    function redeemDXBL(
        ...
        if (wdAmt > vaultBal) {
            ...
            if (vaultBal >= minOutAmount) {
                ...
                revert(
                    "Insufficient asset balance to produce expected withdraw amount"
                );
            }
        }
    ...
    function _initializeFeeTokens(
        VaultStorage.VaultData storage rs,
        VaultStorage.FeeTokenConfig memory config
    ) internal {
        require(
            config.feeTokens.length > 0 &&
                config.feeTokens.length == config.priceFeeds.length,
            "Must provide equal-length arrays for fee tokens and price feeds"
        );
        ...
        require(
            rs.allowedFeeTokens[rs.wrappedNativeToken].decimals > 0,
            "Wrapped native asset must be a valid fee token"
        );
    }
     ...
     */
    function _initializeMintRates(
        VaultStorage.VaultData storage rs,
        VaultStorage.MintRateRangeConfig[] memory ranges
    ) internal {
        require(
            rs.mintRateRanges.length == 0,
            "Already initialized rate ranges"
        );
        for (uint i = 0; i < ranges.length; ++i) {
            ...
            rs.mintRateRanges.push(
                VaultStorage.MintRateRange({
