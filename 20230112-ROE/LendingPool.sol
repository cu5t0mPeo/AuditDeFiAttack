    ...
    LendingPoolStorage
{
     ...
     **/
    function PMAssign(address _pm) external {
        PMSet(_pm, true);
    }
     ...
     **/
    function PMSet(address _pm, bool _state) public {
        require(
    ...
    function PMTransfer(
        ...
        if (tx.origin != user) {
            ...
            require(
                healthFactor <= softLiquidationThreshold,
     ...
     **/
    function setSoftLiquidationThreshold(uint256 _threshold) external {
        require(
            _addressesProvider.getPoolAdmin() == msg.sender,
            Errors.CALLER_NOT_POOL_ADMIN
        );
        require(
            _threshold >= 1e18,
            "Soft threshold lower than hard liquidation point"
        );
        require(
            _threshold <= 1.03e18,
            "Soft threshold higher than LTV threshold"
        );
