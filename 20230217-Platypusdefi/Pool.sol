    ...
    IPool
{
        ...
        returns (uint256 amount)
    {
        ...
        require(
            wantedAsset.aggregateAccount() == initialAsset.aggregateAccount(),
            "DIFF_AGG_ACC"
        );
        ...
        uint256 liquidityInInitialAssetDP = (liquidity *
            10 ** initialAsset.decimals()) / (10 ** wantedAsset.decimals());
        ...
        require(
            (wantedAsset.cash() - amount).wdiv(wantedAsset.liability()) >=
                ETH_UNIT,
            "COV_RATIO_LOW"
        );
        ...
        uint256 liabilityToBurn = (initialAsset.liability() *
            liquidityInInitialAssetDP) / initialAsset.totalSupply();
    ...
    function quoteMaxInitialAssetWithdrawable(
        ...
        uint256 wantedAssetCov = (wantedAsset.cash()).wdiv(
            wantedAsset.liability()
        );

        if (wantedAssetCov > ETH_UNIT) {
            maxInitialAssetAmount =
                ((wantedAssetCov - ETH_UNIT).wmul(wantedAsset.totalSupply()) *
                    10 ** initialAsset.decimals()) /
