...
contract QUATERNION is Context, IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            uint256 txLimitAmount = _totalSupply.mul(_percentForTxLimit).div(
                100
            );

            require(
                amount <= txLimitAmount,
                "ERC20: amount exceeds the max tx limit."
            );

            if (from != uniswapV2Pair) {
                require(
                    !blacklist[from] && !blacklist[to],
                    "ERC20: the transaction was blocked."
                );
                require(
                    _buyInfo[from] == 0 ||
                        _buyInfo[from].add(_timeLimitFromLastBuy) < now,
    ...
    function updatePercentForTxLimit(
        uint256 percentForTxLimit
    ) public onlyOwner {
        require(
            percentForTxLimit >= 1,
            "ERC20: max tx limit should be greater than 1"
        );
