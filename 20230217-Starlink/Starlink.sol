...
contract StarlinkCoin is ERC20, ERC20Burnable, Ownable {
    ...
    uint256 public numTokensSellToAddToLiquidity =
        tokensForRewards.mul(lpNum).div(10000);
    ...
    uint256 public sellFeeRate =
        LPSellFees.add(marketSellFees).add(burnSellFees);
    ...
    constructor() ERC20("Starlink Coin", "Starlink") {
        ...
        uniswapV2Pair = _uniswapV2Factory.createPair(
            address(this),
        ...
        if (recipient == uniswapV2Pair) {
            if (
                ...
                super._transfer(
                    sender,
                    mintContract,
                    _fee.mul(LPSellFees).div(sellFeeRate)
                );
                super._transfer(
                    sender,
                    addressForMarketing,
                    _fee.mul(marketSellFees).div(sellFeeRate)
                );
                super._transfer(
                    sender,
                    BurnAddr,
                    _fee.mul(burnSellFees).div(sellFeeRate)
                );
            ...
            if (
                ...
                super._transfer(
                    sender,
                    mintContract,
                    _fee.mul(LPBuyFees).div(buyFeeRate)
                );
                super._transfer(
                    sender,
                    addressForMarketing,
                    _fee.mul(marketBuyFees).div(buyFeeRate)
                );
                super._transfer(
                    sender,
                    BurnAddr,
                    _fee.mul(burnBuyFees).div(buyFeeRate)
                );
                amount = amount.sub(_fee);
                buyIndex = buyIndex + 1;
            }
    ...
    function SetBuyIndexSellLiquify(
        uint256 newBuyIndexSellLiquify
    ) external onlyOwner {
