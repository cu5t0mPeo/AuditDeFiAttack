    ...
    ISwapExecutor
{
    ...
    function executeSwap(
        ...
        unchecked {
            for (uint256 i = 0; i < swapDescriptions.length; i++) {
                IERC20 sourceToken = swapDescriptions[i].sourceToken;
                uint256 balanceToSwap = sourceToken.universalBalanceOf(
                    address(this)
                );
                if (balanceToSwap == 0) {
                ...
                for (uint256 j = 0; j < swaps.length; j++) {
                    ...
                    uint256 poolSwapAmount = (balanceToSwap * swap.tokenRatio) /
                        _ONE;
                    if (poolSwapAmount == 0) {
                    ...
                    if (swap.data.length > 0) {
                        ...
                        if (swap.callType == CallType.Direct) {
                            uint256 value = performSourceTokenInteractionAndGetEthValue(
                                    sourceToken,
                                    swap.sourceInteraction,
                                    poolSwapAmount,
                                    swap.target
                                );
                            bytes memory data = swap.amountOffset !=
                                type(uint256).max // flag meaning no patching is required
                                ? LowLevelHelper.patchUint(
                                    swap.data,
                                    poolSwapAmount,
                                    swap.amountOffset
                                )
                                : swap.data;
                            (success, result) = swap.target.call{value: value}(
                                data
                            );
                            ...
                            (
                                ...
                                uint256 valueLimit,
                                bytes memory data
                            ) = abi.decode(result, (address, uint256, bytes));
                            uint256 value = performSourceTokenInteractionAndGetEthValue(
                                    sourceToken,
                                    swap.sourceInteraction,
                                    poolSwapAmount,
                            ...
                            revert EnumOutOfRangeValue(
                                EnumType.CallType,
                                uint256(swap.callType)
                            );
                        }
                        if (!success) {
                            string memory reason = RevertReasonParser.parse(
            ...
            address payable feeRecipient = payable(
                address(uint160(feeAndFeeRecipient))
            );
            uint256 decodedFee = feeAndFeeRecipient >> 160;
            uint256 fee = decodedFee > 0 && feeRecipient != address(0)
                ? decodedFee
                : 0;
            uint256 balanceOfThis = tokenToTransfer.universalBalanceOf(
                address(this)
            );
            if (balanceOfThis <= 1) {
    ...
    function performSourceTokenInteractionAndGetEthValue(
        IERC20 sourceToken,
        ...
        // solhint-disable-next-line no-empty-blocks
        if (sourceInteraction == SourceTokenInteraction.None) {} else if (
            sourceInteraction == SourceTokenInteraction.ApproveToTarget
        ) {
            sourceToken.setAllowance(target, poolSwapAmount);
        } else if (
            sourceInteraction == SourceTokenInteraction.TransferToTarget
        ) {
            sourceToken.safeTransfer(target, poolSwapAmount);
        } else {
            revert EnumOutOfRangeValue(
                EnumType.SourceTokenInteraction,
                uint256(sourceInteraction)
            );
        }
        return 0;
    }
