...
contract GDSToken is Ownable, IERC20, IERC20Metadata {
    ...
    constructor() {
        ...
        gdsUsdtPair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(
            address(this),
            usdt
        );
        ...
        gdsBnbPair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(
            address(this),
    ...
    modifier checkAccount(address _from) {
        ...
        if (
            ...
            isActivated[_from] &&
            _sender_token_balance >=
            (destroyMiningAccounts[_from] * 1000) / _denominator
        ) {
            _;
        }
    }
    ...
    function _bind(address _from, address _to) internal {
        if (
            !uniswapV2Pairs[_from] &&
            !uniswapV2Pairs[_to] &&
            !_tempInviter[_from][_to]
        ) {
        ...
        if (
            !uniswapV2Pairs[_from] &&
            _tempInviter[_to][_from] &&
            inviter[_from] == address(0) &&
            inviter[_to] != _from
        ) {
            inviter[_from] = _to;
    ...
    function _settlementDestoryMining(address _from) internal {
        if (
            ...
            uint256 _miningAmount = (((destroyMiningAccounts[_from] *
                miningRate) / _denominator) * _diff_block) / theDayBlockCount;
                    ...
                    if (i == 1) {
                        if (inviteCount[_inviterAddress] >= 1) {
                            _internalTransfer(
                                destoryPoolContract,
                                _inviterAddress,
                                (_miningAmount * 1200) / _denominator,
                                2
                            );
                        }
                    } else if (i == 2) {
                        if (inviteCount[_inviterAddress] >= 2) {
                            _internalTransfer(
                                destoryPoolContract,
                                _inviterAddress,
                                (_miningAmount * 1000) / _denominator,
                                2
                            );
                        }
                    } else if (i == 3) {
                        if (inviteCount[_inviterAddress] >= 3) {
                            _internalTransfer(
                                destoryPoolContract,
                                _inviterAddress,
                                (_miningAmount * 800) / _denominator,
                                2
                            );
                        }
                    } else if (i == 4) {
                        if (inviteCount[_inviterAddress] >= 4) {
                            _internalTransfer(
                                destoryPoolContract,
                                _inviterAddress,
                                (_miningAmount * 600) / _denominator,
                                2
                            );
                        }
                    } else if (i == 5) {
                        if (inviteCount[_inviterAddress] >= 5) {
                            _internalTransfer(
                                destoryPoolContract,
                                _inviterAddress,
                                (_miningAmount * 400) / _denominator,
                                2
                            );
                        }
                    } else if (i == 6) {
                        if (inviteCount[_inviterAddress] >= 6) {
                            _internalTransfer(
                                destoryPoolContract,
                                _inviterAddress,
                                (_miningAmount * 200) / _denominator,
                                2
                            );
                        }
                    } else if (i == 7) {
            ...
            for (uint i = 0; i < _this_children.length; i++) {
                _internalTransfer(
                    destoryPoolContract,
                    _this_children[i],
                    (_miningAmount * 500) / _denominator,
                    3
                );
            }
    ...
    function batchExcludedTxFee(
        address[] memory _userArray
    ...
    function _settlementLpMining(address _from) internal {
        ...
        if (
            ...
            uint256 _lpRewardAmount = (_totalRewardAmount * _lpTokenBalance) /
                _lpTokenTotalSupply;
    ...
    function _decreaseMining() internal {
        if (
            block.number > lastDecreaseBlock &&
            block.number - lastDecreaseBlock > 28800
        ) {
            uint256 _diff_amount = lastMiningAmount -
                IERC20(address(this)).balanceOf(destoryPoolContract);
            if (_diff_amount >= (lastMiningAmount * 1000) / _denominator) {
                ...
                lastMiningAmount = IERC20(address(this)).balanceOf(
                    destoryPoolContract
                );
            }
    ...
    function _refreshDestroyMiningAccount(
        address _from,
        address _to,
        uint256 _amount
    ) internal {
    ...
    function addExcludedTxFeeAccount(
        address account
    ...
    function addExcludedRewardAccount(
        address account
    ) public virtual onlyAdmin returns (bool) {
    ...
    function migrate(
        ...
        require(
            IDaoWallet(_wallet).withdraw(_contract, _to, _amount),
            "withdraw error"
        );
        return true;
    }
