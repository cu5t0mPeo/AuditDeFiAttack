...
contract ExchangeWithOrionPool is Exchange, IPoolSwapCallback {
    ...
    function safeAutoTransferFrom(
        ...
        require(
            msg.sender == _orionpoolRouter,
            "Only _orionpoolRouter allowed"
        );
    ...
    function fillThroughOrionPool(
        ...
        require(
            checkPosition(order.senderAddress),
            order.buySide == 0 ? "E1PS" : "E1PB"
        );
    }
    ...
    function withdrawToPool(
        ...
        (uint amountA, uint amountB) = LibPool.doWithdrawToPool(
            assetA,
            asseBNotETH,
            amountADesired,
            amountBDesired,
