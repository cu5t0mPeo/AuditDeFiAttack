...
library LibExchange {
    ...
    function _updateBalance(
        ...
        if (amount > 0 && beforeBalance < 0) {
            ...
            if (asset != address(0)) {
                afterBalance += int(
                    _tryDeposit(asset, uint(-1 * afterBalance), user)
                );
            }
        ...
        if (beforeBalance != afterBalance) {
            require(
                afterBalance >= type(int192).min &&
                    afterBalance <= type(int192).max,
                "E11"
            );
    ...
    function _tryDeposit(
        ...
        uint256 amountInBase = uint256(
            LibUnitConverter.decimalToBaseUnit(asset, amount)
        );
    ...
    function creditUserAssets(
        ...
        require(
            (amount >= 0 && remainingAmount >= beforeBalance) ||
                (amount < 0 && remainingAmount < beforeBalance),
            "E11"
        );
        int sentAmount = 0;
        ...
        if (tradeType == 1 && amount > 0 && remainingAmount > 0) {
            uint amountInBase = uint(
                LibUnitConverter.decimalToBaseUnit(asset, uint(amount))
            );
            uint contractBalance = asset == address(0)
    ...
    function updateOrderBalanceDebit(
        ...
        {
            //  Stack too deep
            bool isCorrectFee = ((flags & kCorrectMatcherFeeByOrderAmount) !=
                0);
