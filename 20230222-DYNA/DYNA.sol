...
contract Dynamic is Context, IBEP20, Ownable {
    ...
    function transferTokens(uint256 tokenBalance) private lockTheSwap {
        ...
        uint256 liquidityCapacity = newBalance.mul(defaultLiquidity).div(
            defaultLiquidity.add(defaultTreasury.mul(2)).add(
                defaultMarketing.mul(2)
            )
        ); // liquidity = 1/3 total
    ...
    function _setMaxSoldAmount(uint256 maxvalue) external onlyOwner {
        require(
            maxvalue >= 10 ** 6 * 10 ** 18,
            "_maxSoldAmount must be greater than or equal to 0.1% total supply"
        );
    ...
    function _setTreasuryWalletAddress(
        address treasuryWalletAddr
    ) external onlyOwner {
        _treasuryWallet = payable(treasuryWalletAddr);
    }

    function _setMarketingWalletAddress(
